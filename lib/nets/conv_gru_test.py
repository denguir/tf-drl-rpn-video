...
class NetworkVideo():
  ...
  def run(self, sess, rl_in, rl_hid, aux_info):
    ...
    sess.run(init)
    sess.run(done_prob, feed_dict={self._rl_in : rl_in, self._rl_hid : rl_hid, self._aux_done_info : aux_info})
    sess.run(fix_prob, feed_dict={self._rl_in : rl_in, self._rl_hid : rl_hid, self._aux_done_info : aux_info})
    sess.run(done_logits, feed_dict={self._rl_in : rl_in, self._rl_hid : rl_hid, self._aux_done_info : aux_info})
    sess.run(fix_logits, feed_dict={self._rl_in : rl_in, self._rl_hid : rl_hid, self._aux_done_info : aux_info})
    sess.run(h, feed_dict={self._rl_in : rl_in, self._rl_hid : rl_hid, self._aux_done_info : aux_info})

    print(done_prob.shape)
    print(fix_prob.shape)

...
import subprocess
import uuid
from .paris_eval import paris_eval
from model.config import cfg


class paris(imdb):
  def __init__(self, image_set, use_diff=False):
    name = 'paris_' + image_set
    if use_diff:
      name += '_diff'
    imdb.__init__(self, name)
    self._image_set = image_set
    self._devkit_path = self._get_default_path()
    self._data_path = os.path.join(self._devkit_path, 'Paris')
  ...
  def image_path_from_index(self, index):
    ...
    image_path = os.path.join(self._data_path, 'JPEGImages',
                              'ParisAttack' + index + self._image_ext)
  ...
  def _get_default_path(self):
    ...
    """
    return os.path.join(cfg.DATA_DIR, 'ParisDevKit')
  ...
  def gt_roidb(self):
    ...
    gt_roidb = [self._load_paris_annotation(index)
  ...
  def _load_paris_annotation(self, index):
    """
  ...
  def _get_paris_results_file_template(self):
  ...
  def _write_paris_results_file(self, all_boxes):
    ...
    for cls_ind, cls in enumerate(self.classes):
      if cls == '__background__':
        continue
      print('Writing {} Paris results file'.format(cls))
      filename = self._get_paris_results_file_template().format(cls)
  ...
  def evaluate_detections(self, all_boxes, output_dir, start_idx=None,
                          end_idx=None):
    self._write_paris_results_file(all_boxes)

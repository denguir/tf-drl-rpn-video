...
import subprocess
import uuid
from .pennfudanped_eval import pennfudanped_eval
from model.config import cfg


class pennfudanped(imdb):
  def __init__(self, image_set, use_diff=False):
    name = 'pennfudanped_' + image_set
    if use_diff:
      name += '_diff'
    imdb.__init__(self, name)
    self._image_set = image_set
    self._devkit_path = self._get_default_path()
    self._data_path = os.path.join(self._devkit_path, 'PennFudanPed')
  ...
  def _get_default_path(self):
    ...
    """
    return os.path.join(cfg.DATA_DIR, 'PennFudanPedDevKit')
  ...
  def gt_roidb(self):
    ...
    gt_roidb = [self._load_pennfudanped_annotation(index)
  ...
  def _load_pennfudanped_annotation(self, index):
    """
  ...
  def _get_pennfudanped_results_file_template(self):
  ...
  def _write_pennfudanped_results_file(self, all_boxes):
    for cls_ind, cls in enumerate(self.classes):
      if cls == '__background__':
        continue
      print('Writing {} PennFudanPed results file'.format(cls))
  ...
  def evaluate_detections(self, all_boxes, output_dir, start_idx=None,
                          end_idx=None):
    self._write_pennfudanped_results_file(all_boxes)
